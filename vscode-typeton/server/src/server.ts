...
import { TypetonLexer } from './grammar/TypetonLexer';
import { TypetonParser } from './grammar/TypetonParser';
...
interface BuiltInCompletionDefinition {
    label: string;
...
const keywordDefinitions: BuiltInCompletionDefinition[] = [
    {
        label: 'var',
...
const typeDefinitions: BuiltInCompletionDefinition[] = [
    {
        label: 'Int',
...
const printFunction: BuiltInCompletionDefinition = {
    label: 'print',
...
const mathFunctionDefinitions: BuiltInCompletionDefinition[] = [
    {
        label: 'ABS',
        kind: CompletionItemKind.Function,
...
const motionCommandDefinitions: BuiltInCompletionDefinition[] = [
    {
        label: 'move_abs',
        kind: CompletionItemKind.Function,
...
const encoderMethodDefinitions: BuiltInCompletionDefinition[] = [
    {
        label: 'waitForAngle',
        kind: CompletionItemKind.Function,
...
const inputHelperDefinitions: BuiltInCompletionDefinition[] = [
    {
        label: 'Input.setType',
...
const runtimeConstantDefinitions: BuiltInCompletionDefinition[] = [
    {
        label: 'X',
...
const axisPropertyDefinition: BuiltInCompletionDefinition = {
    label: 'Encoder',
    kind: CompletionItemKind.Class,
...
const encoderPropertyDefinitions: BuiltInCompletionDefinition[] = [
    {
        label: 'Angle',
        kind: CompletionItemKind.Constant,
        detail: 'Float',
...
const inputDotMethodDefinitions: BuiltInCompletionDefinition[] = [
    {
        label: 'setType',
        kind: CompletionItemKind.Method,
        detail: 'Configure input polarity',
...
function getDocumentState(document: TextDocument): DocumentState {
...
function parseDocument(document: TextDocument): DocumentState {
    ...
    ParseTreeWalker.DEFAULT.walk(symbolTable, tree);

    const syntaxDiagnostics = errorListener.getDiagnostics();
    const semanticDiagnostics =
        syntaxDiagnostics.length === 0 ? runSemanticDiagnostics(document) : [];
...
function runSemanticDiagnostics(document: TextDocument): Diagnostic[] {
    ...
    try {
        const result = spawnSync(typetonExecutable, ['compile', tempFile.path], {
            encoding: 'utf8',
        ...
        connection.console.warn(`Typeton semantic diagnostics failed: ${(error as Error).message}`);
        return [];
    } finally {
        tempFile.cleanup();
    }
...
function createTempDocument(content: string): { path: string; cleanup: () => void } {
...
function handleSemanticSpawnError(error: NodeJS.ErrnoException): void {
    if (error.code === 'ENOENT' && !semanticChecksDisabled) {
        semanticChecksDisabled = true;
        connection.console.warn(
            `Typeton CLI '${typetonExecutable}' was not found on PATH. Semantic diagnostics have been disabled.`
        );
    }
}

function parseCompilerDiagnostics(output: string, document: TextDocument): Diagnostic[] {
...
function extractRangeFromLocation(
...
function clampPosition(document: TextDocument, line: number, character: number): Position {
    ...
    return Position.create(safeLine, safeChar);
}
...
function createSymbolCompletions(symbolTable: SymbolTable): CompletionItem[] {
    const builder: CompletionItem[] = [];
    for (const symbol of symbolTable.getAllSymbols()) {
        builder.push({
            label: symbol.name,
            kind: symbol.kind === 'function' ? CompletionItemKind.Function : CompletionItemKind.Variable,
            detail: symbol.type,
            documentation: symbol.documentation
                ? { kind: MarkupKind.Markdown, value: symbol.documentation }
                : undefined
        });
    }
    return builder;
...
function mergeCompletions(builtIns: CompletionItem[], custom: CompletionItem[]): CompletionItem[] {
    const seen = new Set<string>(builtIns.map((item) => item.label.toLowerCase()));
...
function buildHoverDocumentMap(items: CompletionItem[]): Map<string, { kind: MarkupKind; value: string }> {
    const map = new Map<string, { kind: MarkupKind; value: string }>();
...
function toCompletionItem(definition: BuiltInCompletionDefinition): CompletionItem {
    return {
...
function getDotCompletionContext(document: TextDocument, position: Position): DotCompletionContext | null {
    const offset = document.offsetAt(position);
    if (offset <= 0) {
        return null;
    }
...
function provideDotCompletion(context: DotCompletionContext, state: DocumentState): CompletionItem[] {
...
function getAxisDotCompletions(path: string[]): CompletionItem[] {
    if (path.length === 1) {
...
function resolveStructType(symbolTable: SymbolTable, path: string[]): string | null {
    if (path.length === 0) {
        return null;
    }
...
function normalizeTypeForLookup(type?: string | null): string | null {
    if (!type) {
        return null;
    }
...
function getStructFieldCompletions(symbolTable: SymbolTable, structType: string, partial: string): CompletionItem[] {
...
function filterCompletions(items: CompletionItem[], partial: string): CompletionItem[] {
    if (!partial) {
        return items;
    }
...
class DiagnosticErrorListener implements ParserErrorListener {
    ...
    public syntaxError<T extends Token>(
        ...
        msg: string,
        _ex: RecognitionException | undefined
    ): void {
    ...
    private createDiagnosticRange(
        ...
        const fallbackRange = Range.create(fallbackStart, fallbackEnd);
    ...
    private getTokenOffsets(token: Token): { start: number; end: number } | null {
    ...
    private rangeBeforeOffset(offset: number): Range | null {
    ...
    private findPreviousTokenBounds(initialOffset: number): { start: number; end: number } | null {
    ...
    private rangeForTrailingToken(): Range | null {
    ...
    private isEndOfFileToken(token: Token): boolean {
        return token.type === Token.EOF || token.text === '<EOF>';
    }
}
